...
print "\nYou are running foxy_qtl_pipeline_known_markers.py pipeline program version " + version + "\n"
...
if re.match('y', comparison):
    ...
    for t in traits:
        ...
        if not os.path.exists(s.join(seq)):
            ...
            s = "/"
            seq = (pwd, outfile, t, 'comparison', 'known_markers.out')
            print s.join(seq)
            os.makedirs(s.join(seq))
            ...
            for c in comp:
                ...
                cond_trait_dir_so = trait_dir + "/" + c + "/" + "known_markers.out"
    ...
    for p in phe_r:
        ...
        s = "/"
        seq = (pwd, outfile, p, "known_markers.out")
...
### Perform scanone, scantwo and MQM QTL analysis
...
if re.match('y', server):
    ...
    if re.match('y', comparison):
        def parallel_comp(x):
            ...
            qtlcommand_raw_phenotype = "Rscript do.qtl.comp_known_markers.R " + str(phe_r_col_number) + " " + treat + " " + trait + " " + outfile + " " +  trait_model + " " + qtl_method
        ...
        def parallel_diff(x):    
            ...
            qtlcommand_diff_phenotype = "Rscript do.qtl_known_markers.R " + str(phe_d_col_number) + " " + outfile + " " + trait + " " + " cross.object.diff.Rdata" + " " +  trait_model + " " + qtl_method
        ...
        def parallel_raw(x):
            ...
            qtlcommand_raw_phenotype = "Rscript do.qtl_known_markers.R " + str(phe_r_col_number) + " " + outfile + " " + trait + " cross.object.raw.Rdata" + " " +  trait_model + " " + qtl_method
            print qtlcommand_raw_phenotype
            subprocess.call(qtlcommand_raw_phenotype, shell=True)
        # Call analysis of each trait in parallel    
    ...
    if re.match('y', comparison):
        ...
        for p in phe_r:
            ...
            print "This is phe_r column number: " +str(phe_r_col_number)
            qtlcommand_raw_phenotype = "Rscript do.qtl_known_markers.R " + str(phe_r_col_number) + " " + outfile + " " + p + " " + " cross.object.diff.Rdata" + " " +  trait_model + " " + qtl_method
            print qtlcommand_raw_phenotype
            subprocess.call(qtlcommand_raw_phenotype, shell=True)
            phe_r_col_number += 1
...
if re.match('y', server):
    ...
    if re.match('y', comparison):
        def parallel_get_gene_comp(x):
            ...
            # Build command
            qtlcommand_get_gene_comp = "Rscript get.gene.comp_known_markers.R " + trait_dir_treat + " " + trait + " " + treat
        ...
        def parallel_get_gene_diff(x):
            ...
            # Build command
            qtlcommand_get_gene_diff = "Rscript get.gene_known_markers.R " + trait_dir_comp + " " + diff_trait
        ...
        def parallel_get_gene(x):
            ...
            # Build a text command to invoke in the shell
            qtlcommand_get_gene = "Rscript get.gene_known_markers.R " + trait_dir + " " + trait
    ...
    if re.match('y', comparison):
        for t in traits:
            ...
            for c in comp:
                ...
                if os.path.exists(cond_trait_dir):
                    cond_trait_dir_so_qtl = cond_trait_dir + "/" + "known_markers.out" + "/" + "summary.table.known_markers." + c + "." + t + ".csv"
        ...
        for p in phe_r:
            ...
            s = "/"
            seq = (pwd, outfile, p, "known_markers.out", "summary.table.known_markers." + p + ".csv")
