...
if (workspace == "cross.object.diff.Rdata") {
  ...
  dir_known_markers<-paste(dirpname, "known_markers.out", sep="/")
...
if (workspace == "cross.object.raw.Rdata") {
  ...
  dir_known_markers<-paste(dir.stem,pname,"scanone.out", sep="/")
...
pdf(file=paste(dir_known_markers,"/QTL.interactionplot.",pname, ".pdf", sep=""))
...
if (nrow(known_markers) > 0 ) {
    ...
    assign(paste('known_qtl.out.fitqtl', pname, sep="."), out.fitqtl)
...
rm(limit.so,max.out.so, max.lod.so, max.perm.so, operm.so, out.so, pname, i, dir_known_markers, dir, args)
save.image(paste(dirpname, session_image_name, sep="/"))
rm(list=ls())
