...
dir.trait.treat<-args[1]
trait<-args[2]
treat<-args[3]

# specify directory path for MQM and Scanone results
path.mqm<-paste(dir.trait.treat, '/mqm.out', sep="")
path.km<-paste(dir.trait.treat, '/known_markers.out', sep="")

# specify path of MQM file
mqm.qtl<-paste(dir.trait.treat, '/mqm.out/', 'summary.table.mqm.', treat, ".", trait, ".csv", sep="")
km.qtl<-paste(dir.trait.treat, '/known_markers.out/', 'summary.table.so.', treat, ".", trait, ".csv", sep="")
...
if (file.exists(mqm.qtl)) {
  ...
  for(rw in 1:nrow(qtls.mqm)) {    
    ...
    for(n in 1:length(sv.gene.id)) {
      a<-annotation[annotation$locusName %in% sv.gene.id[n],]
      if(nrow(a) > 0) {
        g<-conf_int_genes[n,]
  ...
  write.table(annotation.out, file=annotation.file, quote=F, row.names=F,sep = "/t")
