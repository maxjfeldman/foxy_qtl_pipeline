...
# This script is run if the User specified a treatment comparison
...
# Get arguments 
i <- args[1]
i<-as.numeric(i)  # Trait number
treat <- args[2] # Name of treatment
trait <- args[3]  # Name of trait
outstem <- args[4] # What is the base directory
...
dirscanone<-paste(dir.stem, trait, treat,"scanone.out", sep="/")
...
####################
  ...
  if(length(stepout.a) > 0) {
    ...
    # Proportioning of variance for mqm results (full model)
    ...
    if (length(m.names) > 1) {
      ...
      # Proportioning of variance for mqm results (individual QTL as proportion)
...
# If no QTL found specify that

if (nrow(qtl.table.so) == 0 ) {
  write("No significant QTL", file=paste(dirscanone,"/scanone_phe_vals_and_allele_counts_", pname, ".txt", sep=""))

  # Perform positive stepwise forward and reverse model selection given formula, fine tuned QTL location, and penalites
  # This is an additive model, run with two different penalties that are specified above  stepout.a<-stepwiseqtl(fg.cr.obj, pheno.col=i, penalties=pen_heavy, max.qtl=25, scan.pairs=T, additive.only=T, model=c("normal"))
  assign(paste('stepout.a', pname, sep="."), stepout.a)
  ...
  # Still no QTL? Let the user know...
  if(length(stepout.a) == 0) {
     write("No significant QTL in MQM", file=paste(dirmqm,"/empty_summary.table.mqm.", pname, ".txt", sep=""))
     save.image(paste(dirtrait, session_image_name, sep="/"))
     if(length(stepout.a.lite) > 0) {
...
save.image(file=paste(dirtrait, session_image_name, sep="/" ))
rm(limit.so,max.out.so, max.lod.so, max.perm.so, operm.so, out.so, pname, i, dirscanone, phevalues, trait, treat, dirtrait, dir, args)
rm(list=ls())