...
# It used a set of known markers provided by the User in the base directory. 
# It must be formatted identically to the example and named: 'markers_for_fixed_model.csv'

# This script is relatively unelegant at the moment. Essentially an iteration of other scripts.
...
# Load in data from correct diretory
load(paste(dir.stem, "cross.object.raw.Rdata", sep="/"))

# Which phenotype?
pname<-phenames(fg.cr.obj)[i]

# Specify paths to directories which store data
dir_known_markers<-paste(dir.stem, trait, treat,"known_markers.out", sep="/")
dirmqm<-paste(dir.stem, trait, treat,"mqm.out", sep="/")
dirtrait<-paste(dir.stem, trait, treat, sep="/")
...
write.csv(phevalues, file=paste(dirtrait,"/ordered_phe_vals_", pname, ".csv", sep=""), quote=F, row.names=F)

# perform scanone QTL analysis on a fill.geno cross object using method "hk" and default paramters
out.so <- scanone(fg.cr.obj, pheno.col=i, method=c("hk"))
assign(paste("out.so",pname,sep="."), out.so)

# perform significance test using permutation (1000)
operm.so<-scanone(fg.cr.obj, pheno.col=i, method=c("hk"), n.perm=1000)
...
write.csv(as.data.frame(qtl.table.so), file=paste(dir_known_markers,table.name, sep="/"), quote=F, row.names=T)
assign(paste("qtl.table.so", pname, sep="."), qtl.table.so)

# plot results to file and include permutation result
if (file.exists(dir_known_markers)) {
pdf(file=paste(dir_known_markers,"/scanone.qtls.",pname, ".pdf", sep=""))
plot(out.so, ylim=c(0,limit.so), main=pname, bandcol="gray70")
abline(h=max.perm.so, col="red")
dev.off()

# Print a histogram of phenotype data
pdf(file=paste(dirtrait,"/histogram_of_", pname, ".pdf", sep=""))
...
#pdf(file=paste(dir_known_markers,"/scantwo.qtls.",pname, ".pdf", sep=""))
...
#write.csv(sig.s2.hk, file=paste(dir_known_markers,"/sig_interaction_qtl.", pname, ".csv", sep=""), quote=F)
...
pdf(file=paste(dir_known_markers,"/known_qtl.interactionplot.",pname, ".pdf", sep=""))
...
# Read in a set of known qtl markers
known_markers<-read.csv('markers_for_fixed_model.csv')

save.image(file=paste(dirtrait,  session_image_name, sep="/" ))

# If significant QTL found, print a .csv file which contains allele call at each polymorphism and phenotype value
if (nrow(known_markers) > 0 ) {
  ...
  #write.csv(small.map, file=paste(dir_known_markers,"/scanone_phe_vals_and_allele_counts_", pname, ".csv", sep=""), quote=F)
  ...
  qtl.markers<-known_markers$marker
  chrs<-known_markers$chr
  pos<-known_markers$pos
  save.image(file=paste(dirtrait, session_image_name, sep="/" ))
  
  # Get effect size of scanone QTL
  pdf(file=paste(dir_known_markers,"/known_qtl.effect_size.",pname, ".pdf", sep=""))
  fx.so<-effectscan(fg.cr.obj_sim, pheno.col=i, get.se=T, draw=T)
  dev.off()
  # Write out the effect size for each marker as a .csv file
  write.csv(fx.so, file=paste(dir_known_markers,"/known_qtl.allele_effects_all_markers.", pname, ".csv", sep=""), quote=F)
  assign(paste('known_qtl.allele_effects_all_markers', pname, sep="."), fx.so)
  
  fx.qtl.marker<-fx.so[rownames(fx.so) %in% qtl.markers,]
  # Write out the effect size for qtl markers as a .csv file
  write.csv(fx.qtl.marker, file=paste(dir_known_markers,"/known_qtl.allele_effects_qtl_markers.", pname, ".csv", sep=""), quote=F)
  assign(paste('known_qtl.allele_effects_qtl_markers', pname, sep="."), fx.qtl.marker)
  
  # Make a plot of the PXG
  pdf(file=paste(dir_known_markers,"/known_qtl.phenotypeXgenotype.",pname, ".pdf", sep=""))
  ...
  assign(paste('known_qtl.formula', pname, sep="."), my.formula)
  ...
  out.fitqtl<-fitqtl(fg.cr.obj, pheno.col=i, qtl=qtl, formula=my.formula, method='hk', get.ests=T)
  assign(paste('known_qtl.fitqtl', pname, sep="."), out.fitqtl)
  ...
  assign(paste('known_qtl.full.mdl.var', pname, sep="."), full.mdl.var)
  write.csv(full.mdl.var, file=paste(dir_known_markers,"/known_qtl.prop.var.full.mdl.", pname, ".csv", sep=""), quote=F)
  ...
  if (length(qtl.markers) > 1) {
    ...
    # dropone.mdl.var$prop.variance<-c(100*(dropone.mdl.var[1,1]/full.mdl.var[3,1]), 100*(dropone.mdl.var[2,1]/full.mdl.var[3,1]), 100)
    ...
    # Partition variance within QTL
      ...
      rownames(dropone.mdl.var)[1:length(qtl.markers)]<-as.character(qtl.markers)
      assign(paste('known_qtl.dropone.mdl.var', pname, sep="."), dropone.mdl.var)
      write.csv(dropone.mdl.var, file=paste(dir_known_markers,"/known_qtl.prop.var.dropone.mdl.", pname, ".csv", sep=""), quote=F)
      
    # Get confidence interval (95%) for each QTL:
      #n.QTL<-revqtl$n.qtl
      n.QTL<-nrow(known_markers)
      #CI_L<-c()
      #CI_R<-c()
      #for(q in 1:n.QTL){
      #  lod_int<-lodint(qtl, qtl.index=q)
      ...
      #}
      #CI<-cbind(CI_L, CI_R)
      #assign(paste('known_qtl.confidence_interval', pname, sep="."), CI)
      #write.csv(CI, file=paste(dir_known_markers,"/known_qtl.confidence_interval.", pname, ".csv", sep=""), quote=F)
    ...
    # Build a table that summarizes the results
      summary.table.known_qtl<-cbind(known_markers[,1:ncol(known_markers)], marker.prop.var, fx_size.a, fx_size.se)
      colnames(summary.table.known_qtl)<-c('marker', 'chr', 'pos', 'lod', 'prop.var', 'additive.fx', 'additive.fx_se')
      assign(paste('summary.table.known_qtl', pname, sep="."), summary.table.known_qtl)
      write.csv(summary.table.known_qtl, file=paste(dir_known_markers,"/summary.table.known_qtl.", pname, ".csv", sep=""), quote=F)
      save.image(file=paste(dirtrait, session_image_name, sep="/" ))
  ...
  if (length(qtl.markers) == 1) {
    ...
    write("No paritioning of variance among QTL if only 1 detected.", file=paste(dir_known_markers,"/prop.var.dropone.mdl.", pname, ".txt", sep=""))
    ...
    # Lets get additive effect sizes from the fitqtl model of scanone qtl
      fx_size_fitqtl<-as.data.frame(summary(out.fitqtl)[2])
      fx_size.a<-as.data.frame(fx_size_fitqtl[2:nrow(fx_size_fitqtl),1])
      fx_size.se<-as.data.frame(fx_size_fitqtl[2:nrow(fx_size_fitqtl),2])
      
    # Build a table that summarizes the results
      summary.table.known_qtl<-cbind(known_markers[,1:ncol(known_markers)], as.data.frame(full.mdl.var[1,2]), fx_size.a, fx_size.se)
      colnames(summary.table.known_qtl)<-c('markers', 'chr', 'pos', 'lod', 'prop.var', 'additive.fx', 'additive.fx_se')
      assign(paste('summary.table.known_qtl', pname, sep="."), summary.table.known_qtl)
      write.csv(summary.table.known_qtl, file=paste(dir_known_markers,"/summary.table.known_qtl.", pname, ".csv", sep=""), quote=F)    
      save.image(file=paste(dirtrait, session_image_name, sep="/" ))
  ...
  stepout.a<-stepwiseqtl(fg.cr.obj, pheno.col=i, qtl=qtl, formula=my.formula, method="hk", penalties=pen_heavy, max.qtl=25, scan.pairs=T, additive.only=T, model=c("normal"))
  assign(paste('stepout.a', pname, sep="."), stepout.a)
  stepout.a.lite<-stepwiseqtl(fg.cr.obj, pheno.col=i, qtl=qtl, formula=my.formula, method="hk", penalties=pen_lite, max.qtl=25, scan.pairs=T, additive.only=T, model=c("normal"))
...
####################
  ...
  if(length(stepout.a) == 0) {
    ...
    save.image(paste(dirtrait, session_image_name, sep="/"))
    if(length(stepout.a.lite) > 0) {
      pdf(file=paste(dirtrait,"/mqm.LOD.profile.", pname, ".pdf", sep=""))
      plotLodProfile(stepout.a.lite, showallchr=T, col="grey", main=pname)
      abline(h=max.perm.so, col="red")
      dev.off()  
      save.image(paste(dirtrait, session_image_name, sep="/"))
...
save.image(file=paste(dirtrait, session_image_name, sep="/" ))
rm(limit.so,max.out.so, max.lod.so, max.perm.so, operm.so, out.so, pname, i, dir_known_markers, phevalues, trait, treat, dirtrait, dir, args)
rm(list=ls())